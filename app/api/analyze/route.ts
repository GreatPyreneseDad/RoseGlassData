...
const SOURCES_SELECT = `
  SELECT a.id, n.label AS topic, a.date,
  ...
  WHERE UPPER(n.label) = $1 AND a.date = $2
  GROUP BY a.id, n.label, a.date
  LIMIT 1
`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const cached = await db.query(SOURCES_SELECT, [normalizedTopic, date]);
    ...
    const fresh = await db.query(SOURCES_SELECT, [normalizedTopic, date]);
    if (fresh.rows.length > 0) {
      const row = fresh.rows[0];
      return NextResponse.json({ topic: row.topic, date: row.date, sources: row.sources });
    }

    return NextResponse.json({ error: "No data found for this entity and date" }, { status: 404 });
  } catch (error: unknown) {
