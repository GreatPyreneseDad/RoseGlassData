...
export async function POST(request: NextRequest) {
  try {
    const auth = await checkAuth(request, "connect");
    ...
    if (!varsRes.ok)
      return NextResponse.json({ error: `Census API error: ${varsRes.status}` }, { status: 502 });
    ...
    const moeNames = new Set(varEntries.filter(([k]) => k.endsWith("M") || k.endsWith("MA")).map(([k]) => k));
    ...
    for (const [varName, varDef] of varEntries as [string, CensusVariable][]) {
      ...
      processed.push({
        ...
        concept, predicate_type: varDef.predicateType || "string",
        has_moe, is_moe,
      });
    }
    ...
    const sessionRes = await db.query(
      ...
      [name || `${dataset_id} ${vintage}`, "census", dataset_id, vintage, endpoint,
       processed.length, concepts.size, geographies, auth.api_key_id]
    );
    ...
    for (let i = 0; i < processed.length; i += BATCH) {
      ...
      await db.query(
        `INSERT INTO db_variables (session_id,name,label,concept,predicate_type,has_moe,is_moe) VALUES ${vals}`, flat
      );
    }
    ...
    if (!conceptList.some(c => c.includes("WEALTH") || c.includes("ASSET")))
      absences.push({ domain: "Economic", absence: "Wealth and asset holdings", significance: "Income tracked but not accumulated wealth" });
    if (!conceptList.some(c => c.includes("MENTAL HEALTH")))
      absences.push({ domain: "Health", absence: "Mental health and subjective wellbeing", significance: "Physical disability tracked; psychological health is not" });
    if (!conceptList.some(c => c.includes("FOOD SECURITY")))
      absences.push({ domain: "Basic needs", absence: "Food security", significance: "Housing cost burden tracked; hunger requires a separate survey" });

    const lens_summary = `Built by a federal agency for administrative enumeration. Unit of analysis is the household.${conceptList.some(c => c.includes("RACE")) ? " Race is self-reported categorical — discrete variable for a continuous reality." : ""} MOE coverage: ${moe_coverage}%.`;
